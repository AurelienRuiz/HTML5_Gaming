...
function (_Phaser$Scene) {
  ...
  _createClass(Level3, [{
    ...
    value: function create() {
      ...
      game.currentGame = 'Level3'; // MUSIC

      var music_ambiance = this.sound.add('ambiance_level3');
      music_ambiance.play(); //MAP

      var background = this.add.tileSprite(0, 0, 7680, 960, 'background_level3').setOrigin(0, 0);
      var map = this.make.tilemap({
        key: 'map_level3'
      });
      var tileset = map.addTilesetImage('winter', 'tiles_level3');
      ...
      coinsLayer.forEach(function (coin) {
        _this2.objs.push(coins.create(coin.x, coin.y - 25 - coin.height, 'coins_level3').setScale(1).setOrigin(0));
      }); // PLAYER
      ...
      function collectCoin(player, coin) {
        var music_coins = this.sound.add('coins_music_level3');
        music_coins.play();
    ...
    value: function update() {
      ...
      if (this.cursors.up.isDown && this.cursors.right.isDown && this.player.body.onFloor()) {
        var jump = this.sound.add('jump_music_level3');
        jump.play();
      ...
      if (this.cursors.up.isDown && this.cursors.left.isDown && this.player.body.onFloor()) {
        var _jump = this.sound.add('jump_music_level3');
