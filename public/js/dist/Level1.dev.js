...
function (_Phaser$Scene) {
  ...
  _createClass(Level1, [{
    ...
    value: function create() {
      ...
      game.currentGame = 'Level1'; // MUSIC

      var music_ambiance = this.sound.add('ambiance_level1');
      music_ambiance.play(); //MAP

      var background = this.add.tileSprite(0, 0, 7680, 960, 'background_level1').setOrigin(0, 0);
      var map = this.make.tilemap({
        key: 'map_level1'
      });
      var tileset = map.addTilesetImage('desert', 'tiles_level1');
      ...
      var coinsLayer = map.getObjectLayer('coins')['objects'];
      var coins = this.physics.add.staticGroup();
      this.objs = [];
      coinsLayer.forEach(function (coin) {
        _this2.objs.push(coins.create(coin.x, coin.y - 25 - coin.height, 'coins_level1').setScale(1).setOrigin(0));
      }); // PLAYER
      ...
      function collectCoin(player, coin) {
        var music_coins = this.sound.add('coins_music_level1');
        music_coins.play();
    ...
    value: function update() {
      ...
      if (this.cursors.up.isDown && this.cursors.right.isDown && this.player.body.onFloor()) {
        var jump = this.sound.add('jump_music_level1');
        jump.play();
      ...
      if (this.cursors.up.isDown && this.cursors.left.isDown && this.player.body.onFloor()) {
        var _jump = this.sound.add('jump_music_level1');
